...
const HitsPlaylist: React.FC = () => {
	...
	useEffect(() => {
		const fetchDanceHitsTracks = async () => {
			setLoading(true)
			try {
				const selection = await getSelectionById(3)
				const trackPromises = selection.data.items.map((id: number) =>
					getTrackById(id)
				)
				const danceHitsTracksResponses = await Promise.all(trackPromises)
				const danceHitsTracks = danceHitsTracksResponses.map(
					response => response.data
				)
				dispatch(setTracks(danceHitsTracks))
			} catch (error) {
